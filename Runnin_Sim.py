...
def update_v_after_e(v_list, v_ok: set, assigned_tv: list, curr_time, epoch_len, spc_dict, map_graph):
    ...
    for v in v_nok:
        if not v.passengers:  # This checks if current passengers list is empty https://flexiple.com/check-if-list-is-empty-python/
            # We check this as if the passengers list is empty, the vehicle is idle (otherwise, eventhough it didn't get an assignment, there are people on it, meaning he has a trip to do.
...
def running_ny_sim(csv_path, num_of_vehicles, num_of_epochs, epoch_len_sec, starting_time=None):
    ...
    logging.basicConfig(filename=str(datetime.datetime.now()).replace(':', '_') + '.log',
                        level=logging.INFO)  # Difference from above line makes it work on Windows - can't use ':' in filename in windows
    ...
    logging.info(
        'This run : ST= : ' + starting_time + ', epoch_len = ' + str(epoch_len_sec) + ' , num of epochs = ' + str(
            num_of_epochs) + ", num of vehicles = " + str(num_of_vehicles) + '\n')
    # def running_ny_sim(csv_path, num_of_vehicles, num_of_epochs, epoch_len_sec, starting_time=None):
    epochs = epoch_separator(requests_csv_path=csv_path, epoch_len_sec=epoch_len_sec, num_of_epochs=num_of_epochs,
                             starting_time=starting_time, spc_dict=spc_dict, map_graph=map_graph)
    ...
    for count, epoch in enumerate(epochs):
        ...
        rv = RV_graph.RV_graph(requests_list=epoch, vehicle_list=v_list, virtual_vehicle=virtual_v, map_graph=map_graph,
                               current_time=curr_time, spc_dict=spc_dict)
        rtv = RTV_graph.RTV_graph(rv_graph=rv, spc_dict=spc_dict, map_graph=map_graph,
